...
class GrowRendererHTML {
  components = {
    Container: function () {
      ...
      if (hasBorder) {
        const borderColor = this.decoration.border.color
        const colorStr = borderColor
          ? `rgba(${borderColor.red}, ${borderColor.green}, ${borderColor.blue}, ${borderColor.alpha || 1})`
          : '#000000'
        styles.border = `${borderWidth}px solid ${colorStr}`
      }
    ...
    Text: function () {
      ...
      return `<span style="color: ${colorStr}">${escapeHTML(text)}</span>`
    }
  }
