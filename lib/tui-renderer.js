...
class GrowRendererTUI {
    ...
    Pressable: function () {
      ...
      if (this.hotkey) {
        this.renderer.registerHotkey(this.hotkey, () => {
          // this.child.decoration = new BoxDecoration({
          //   border: Border.all({ color: Color.from('#f00') })
          // })
          this.onPress()

          this.renderer.draw.call(this.renderer)
        })
      }
  ...
  registerHotkey(hotkey, fn) {
    if (this.hotkeys.has(hotkey)) return

    this.hotkeys.set(hotkey, fn)
  }
  ...
  setup() {
    ...
    if (process.stdin.isTTY) {
      process.stdin.on('data', (data) => {
        ...
        for (const [hotkey, fn] of this.hotkeys.entries()) {
