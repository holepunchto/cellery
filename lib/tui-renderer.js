...
class GrowRendererTUI {
  ...
  components = {
    ...
    Scrollable: function () {
      ...
      // We need to measure the first child to determine item height
      let itemHeight = 1
      if (this.children.length > 0) {
        const firstChildGrid = this.renderer._renderComponent(this.children[0], {
          parent: { width, height }
        })
        if (firstChildGrid) {
      ...
      const { start, end } = this.getVisibleRange(viewportItemCount)

      // Store metadata for scroll indicators (optional, renderers can use this)
      this._scrollInfo = {
        canScrollUp: start > 0,
        canScrollDown: end < this.children.length,
      ...
      const childParent = {
        width: width,
        height: height // Each child gets full width, but we'll clip vertically
      }
      ...
      for (let i = start; i < end; i++) {
        ...
        // Calculate how much of this child fits in remaining viewport
        const remainingHeight = height - currentY
        const childHeight = Math.min(childGrid.length, remainingHeight)

        // Clip child grid to fit viewport
        const clippedChildGrid = childGrid.slice(0, childHeight)

        this.renderer._mergeIntoBuffer(grid, clippedChildGrid, 0, currentY)
